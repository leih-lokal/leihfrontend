...
<main>
  ...
  <section class="container mx-auto px-4 md:px-8 py-8 md:py-12">
    <div class="gallery-grid grid <?= $gridClass ?> gap-4 md:gap-6 <?= $layout ===
 "masonry"
   ? "masonry"
   : "" ?>">
      <?php foreach ($page->images() as $image): ?>
      <div class="gallery-item <?= $enableCategories &&
      $image->category()->isNotEmpty()
        ...
        <?php if (
          ...
          ($image->caption()->isNotEmpty() ||
            $image->photographer()->isNotEmpty() ||
            $image->date()->isNotEmpty())
        ): ?>
        <div class="mt-3 text-sm space-y-1">
          ...
          <p class="text-gray-600">Foto: <?= $image
            ->photographer()
            ->esc() ?></p>
          <?php endif; ?>
          <?php if ($image->date()->isNotEmpty()): ?>
          <p class="text-gray-500"><?= $image->date()->toDate("d.m.Y") ?></p>
          <?php endif; ?>
        </div>
        <?php endif; ?>
      </div>
    ...
    <div class="container mx-auto px-4 md:px-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
        <?php foreach ($page->children()->listed() as $subpage):
          $thumbnail =
            $subpage->images()->first() ?? $subpage->cover()->toFile(); ?>
        <a href="<?= $subpage->url() ?>"
