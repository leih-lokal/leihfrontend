...
<main class="container mx-auto">
  ...
  <?php if ($page->show_banner()->toBool() && ($banner = $page->banner_image()->toFile())): ?>
    ...
    <div class="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent flex items-end">
      <div class="w-full px-6 md:px-8 pb-6 text-white">
        ...
        <?php if ($page->subtitle()->isNotEmpty()): ?>
          <p class="text-lg md:text-xl text-white/90 mt-2"><?= $page->subtitle()->esc() ?></p>
        <?php endif ?>
      </div>
    </div>
  </section>
  ...
  <div class="flex flex-col lg:flex-row gap-8 px-4 md:px-8 py-8 md:py-12">
    ...
    <div class="flex-1 lg:w-2/3">
      ...
      <?php if ($page->content_sections()->isNotEmpty()): ?>
        <?php foreach ($page->content_sections()->toStructure() as $index => $section):
          $sectionImage = $section->image()->toFile();
          $imagePos = $section->image_position()->value();
        ?>
        <section class="mb-12 <?= $index > 0 ? 'pt-8 border-t border-gray-200' : '' ?>">

          <?php if ($section->heading()->isNotEmpty()): ?>
          <h2 class="text-2xl md:text-3xl font-bold mb-6"><?= $section->heading()->esc() ?></h2>
          <?php endif ?>
          ...
          <?php if ($sectionImage && ($imagePos === 'left' || $imagePos === 'right')): ?>
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div class="<?= $imagePos === 'left' ? 'md:order-1' : 'md:order-2' ?>">
              <img src="<?= $sectionImage->url() ?>"
                   alt="<?= $section->heading()->esc() ?>"
                   class="w-full h-auto shadow-md">
            </div>
            <div class="md:col-span-2 <?= $imagePos === 'left' ? 'md:order-2' : 'md:order-1' ?>">
      ...
      <section class="mt-12 pt-8 border-t border-gray-200">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <?php foreach ($page->children()->listed() as $subpage):
            $cover = $subpage->cover()->toFile() ?? $subpage->banner_image()->toFile() ?? $subpage->hero_image()->toFile();
          ?>
          <a href="<?= $subpage->url() ?>" class="group border border-gray-200 rounded-lg overflow-hidden hover:border-leihlokal-500 hover:shadow-md transition-all">
            ...
            <div class="p-4 md:p-6">
              ...
              <?php elseif ($subpage->subtitle()->isNotEmpty()): ?>
                <p class="text-gray-600 text-sm line-clamp-2"><?= $subpage->subtitle()->excerpt(120) ?></p>
              <?php endif ?>
    ...
    <aside class="lg:w-1/3 space-y-6">
      ...
      <div class="bg-gray-50 border border-gray-200 p-4 md:p-6 sticky top-8">
        <?php if ($page->sidebar_heading()->isNotEmpty()): ?>
        <h3 class="font-bold text-lg mb-4"><?= $page->sidebar_heading()->esc() ?></h3>
        <?php endif ?>
        <div class="prose prose-sm">
      ...
      <!-- Quick Links -->
      <?php if ($page->quick_links()->isNotEmpty()): ?>
      <div class="border border-gray-200 p-4 md:p-6">
        <h3 class="font-bold text-lg mb-4">Schnelllinks</h3>
        <ul class="space-y-2">
          <?php foreach ($page->quick_links()->toStructure() as $link): ?>
          <li>
            <a href="<?= $link->url()->esc() ?>"
               class="text-leihlokal-500 hover:text-leihlokal-600 hover:underline flex items-center">
